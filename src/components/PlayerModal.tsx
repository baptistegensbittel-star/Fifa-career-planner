    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          <label className="relative flex flex-col gap-1 text-sm text-gray-400">
            Nom
            <input
              autoFocus
              value={name}
                ...
                {suggestions.map((p) => (
                  <li key={`${p.name}|${p.club}`}>
                    <button
                      ...
                      className="flex w-full items-center justify-between px-2 py-1.5 text-left text-base text-gray-200 hover:bg-white/5"
                    >
                      <span>{p.name}</span>
          ...
          {confirmingDelete && player ? (
            <div className="mt-2 flex items-center justify-between rounded border border-red-500/20 bg-red-500/5 px-3 py-2">
              <span className="text-base text-gray-300">Supprimer {player.name} ?</span>
