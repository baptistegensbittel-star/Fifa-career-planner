  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="mt-3 flex flex-col gap-3">
          <label className="flex flex-col gap-1 text-sm text-gray-400">
            ...
            <input
              ...
              className="w-full max-w-xs rounded border border-white/10 bg-black/20 px-2 py-1.5 text-base text-white outline-none focus:border-[#5b8cff]/60"
            />
          </label>
          ...
          <div className="flex flex-col gap-1 text-sm text-gray-400">
            ...
            {activeCareer.trophies.length > 0 && (
              <ul className="mb-1 flex flex-col divide-y divide-white/5 border border-white/10">
                {activeCareer.trophies.map((t, i) => (
                  <li key={`${t}-${i}`} className="flex items-center justify-between px-2 py-1.5 text-base text-gray-200">
                    {t}
                    <button
                      type="button"
            ...
            <div className="flex gap-2">
              ...
              <button
                type="button"
                onClick={addTrophy}
                className="rounded border border-white/10 px-3 py-1.5 text-base text-gray-300 hover:bg-white/5"
              >
                Ajouter
              </button>
            </div>
      ...
      <div className="flex flex-col gap-2">
        <h2 className="text-sm uppercase tracking-wide text-gray-400">Historique</h2>
        {activeCareer.seasonHistory.length === 0 ? (
          <p className="text-base text-gray-400">Aucune saison archivée pour l'instant.</p>
        ) : (
          <ul className="flex flex-col gap-2">
            {[...activeCareer.seasonHistory].reverse().map((s) => (
              <li key={s.id} className="border border-white/10 p-3">
                <div className="flex items-start justify-between">
                  <div className="text-base font-medium text-white">{s.season}</div>
                  <button
                    type="button"
                ...
                {s.position && <div className="mt-0.5 text-sm text-gray-400">{s.position}</div>}
                {s.trophies.length > 0 ? (
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {s.trophies.map((t, i) => (
                      <span
                        key={`${t}-${i}`}
                        className="rounded bg-white/5 px-2 py-0.5 text-sm text-gray-300"
                      >
                        {t}
                      </span>
                    ))}
                  ...
                  <div className="mt-2 text-sm text-gray-400">Aucun trophée noté.</div>
                )}
              </li>
            ))}
          </ul>
