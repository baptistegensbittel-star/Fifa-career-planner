    ...
    <div className="flex flex-col gap-3">
      ...
      >
        <table className="w-full min-w-[720px] border-collapse text-base">
          <thead>
            <tr className="border-b border-white/10 bg-white/[0.03] text-left text-sm uppercase tracking-wide text-gray-400">
          ...
          <tbody>
            {POSITIONS.map((pos) => (
              <tr key={pos.code} className="border-b border-white/5 align-top last:border-0">
                <td className="px-3 py-2 text-sm font-semibold text-gray-300">
                  <div>{pos.code}</div>
                  <div className="text-xs font-normal text-gray-400">{pos.label}</div>
                </td>
                {DEPTH_CATEGORIES.map((d) => (
